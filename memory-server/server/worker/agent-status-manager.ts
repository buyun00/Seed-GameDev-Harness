...
const AGENT_DEFINITIONS: Array<Omit<AgentState, 'status' | 'currentTask' | 'progress' | 'startedAt' | 'updatedAt'>> = [
...
export class AgentStatusManager {
  ...
  private taskQueue: Array<{ agentId: AgentId; task: string }> = []
  ...
  setStatus(id: AgentId, status: AgentStatus, task = ''): void {
  ...
  completeWork(agentId: AgentId, result = ''): void {
  ...
  private processQueue(agentId: AgentId): void {
  ...
  private broadcast(agent: AgentState): void {
