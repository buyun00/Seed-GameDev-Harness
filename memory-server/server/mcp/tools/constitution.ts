...
export function registerConstitutionTools(server: McpServer, ctx: AppContext) {
  const analyzer = new ConstitutionAnalyzer(ctx)

  server.tool(
    'get_constitution_analysis',
    'Get the current constitution analysis status and extracted rules',
    {},
    async () => {
      const cached = await ctx.cache.get<ConstitutionAnalysisCache>('constitution-analysis')
      if (!cached) {
        return { content: [{ type: 'text' as const, text: JSON.stringify({ status: 'none', rules: [] }) }] }
      }
  ...
  server.tool(
    'run_constitution_analysis',
    'Analyze CLAUDE.md files and extract structured rule blocks',
    { force: z.boolean().optional().describe('Force re-analysis even if up-to-date') },
    async ({ force }) => {
      if (!force) {
        const cached = await ctx.cache.get<ConstitutionAnalysisCache>('constitution-analysis')
        if (cached) {
          const freshness = await analyzer.checkFreshness(cached)
          if (freshness.fresh) {
            return { content: [{ type: 'text' as const, text: JSON.stringify({ status: 'up_to_date', analyzedAt: cached.analyzedAt }) }] }
          }
        }
      }
      const result = await analyzer.analyze()
      await ctx.cache.set('constitution-analysis', result)
      return { content: [{ type: 'text' as const, text: JSON.stringify({ status: 'analyzed', ...result.statusSummary, analyzedAt: result.analyzedAt }) }] }
    },
  )

  server.tool(
    'propose_constitution_edit',
    'Propose an edit to an existing constitution rule block',
    {
    ...
    async ({ ruleId, changes, editIntent }) => {
      const cached = await ctx.cache.get<ConstitutionAnalysisCache>('constitution-analysis')
      if (!cached) {
        return { content: [{ type: 'text' as const, text: JSON.stringify({ error: 'No analysis. Run analysis first.' }) }] }
      }
      ...
      if (!existsSync(absPath)) {
        return { content: [{ type: 'text' as const, text: JSON.stringify({ error: 'Source file not found' }) }] }
      }
  ...
  server.tool(
    'propose_new_rule_block',
    'Propose adding a new rule block to a constitution file',
    {
