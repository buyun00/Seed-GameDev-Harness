...
async function proxyGet(port: number, path: string): Promise<unknown> {
  const resp = await fetch(`http://127.0.0.1:${port}${path}`)
  return resp.json()
}

async function proxyPost(port: number, path: string, body?: unknown): Promise<unknown> {
  const resp = await fetch(`http://127.0.0.1:${port}${path}`, {
    method: 'POST',
