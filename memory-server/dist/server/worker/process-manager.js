...
import { existsSync, readFileSync, writeFileSync, unlinkSync, mkdirSync, readdirSync, realpathSync, openSync, closeSync, } from 'node:fs';
...
export function removePidFile(canonicalPath) {
    const pidPath = getWorkerPidPath(canonicalPath);
    try {
        unlinkSync(pidPath);
    }
...
export function acquirePidFile(canonicalPath, info) {
    ensureWorkersDir();
    const pidPath = getWorkerPidPath(canonicalPath);
...
export function updatePidFile(canonicalPath, info) {
    const pidPath = getWorkerPidPath(canonicalPath);
    writeFileSync(pidPath, JSON.stringify(info, null, 2), 'utf-8');
}
...
export function validatePidFile(canonicalPath) {
...
export function listAllWorkers() {
    ...
    try {
        ...
        for (const file of files) {
            ...
            try {
                const info = JSON.parse(readFileSync(fullPath, 'utf-8'));
