...
const VALID_RULE_CATEGORIES = new Set(RULE_CATEGORY_ORDER);
const RULE_CATEGORY_DESCRIPTIONS = {
...
export function buildConstitutionFilePrompt(file) {
...
export function buildConstitutionComparisonPrompt(category, rules) {
...
export function parseConstitutionAnalysisResult(rawResult, file) {
...
export function parseConstitutionComparisonResult(rawResult, extractedRules) {
...
export function summarizeConstitutionRules(rules) {
    return {
...
export function finalizeComparedRules(rules) {
...
function repairCommonJsonIssues(raw) {
    return escapeUnescapedInnerQuotes(raw);
}
function escapeUnescapedInnerQuotes(raw) {
    let result = '';
    let inString = false;
    let escaped = false;
    for (let i = 0; i < raw.length; i++) {
...
function isLikelyJsonStringTerminator(raw, startIndex) {
...
function sanitizeRelation(rawRelation, sourceRuleId, inputIds) {
    ...
    if (!type || !VALID_RELATION_TYPES.has(type))
        return null;
    if (!targetRuleId || !inputIds.has(targetRuleId) || targetRuleId === sourceRuleId)
        return null;
    return {
        type,
        targetRuleId,
...
function normalizeComparisonRelations(status, relations) {
    return dedupeRelations(relations.filter(relation => {
        if (status === 'effective' && relation.type === 'shadowed_by')
            return false;
        if (status === 'shadowed' && relation.type === 'conflicts_with')
            return false;
        if (status === 'conflicting' && relation.type === 'shadowed_by')
            return false;
        return true;
    }));
}
...
function normalizeCategory(value) {
    ...
    return VALID_RULE_CATEGORIES.has(normalized)
        ? normalized
        : undefined;
}
...
function looksCompoundRule(rule) {
    ...
    const numberedSubitems = Math.max([...normalized.matchAll(/(?:\d+[\)\].、]|[①-⑩])/g)].length, [...excerpt.matchAll(/(?:\d+[\)\].、]|[①-⑩])/g)].length);
    if (numberedSubitems >= 2)
        return true;
