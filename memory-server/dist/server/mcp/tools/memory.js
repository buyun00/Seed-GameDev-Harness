...
export function registerMemoryTools(server, ctx) {
    const pathResolver = new MemoryPathResolver(ctx.projectContext);
    const analyzer = new MemoryAnalyzer(ctx, pathResolver);
    server.tool('list_memory_objects', 'List auto memory objects with their index and status', {
        ...
        if (status)
            objects = objects.filter(o => o.status === status);
        return { content: [{ type: 'text', text: JSON.stringify({ ...result, objects }, null, 2) }] };
    });
    server.tool('propose_memory_edit', 'Propose edits to a memory object', {
    ...
    server.tool('reindex_memory', 'Check and report unindexed memory files', {}, async () => {
