...
- researcher 写完报告后不要立即关闭，必须等待 Phase A Gate 通过，并等待对应 builder 完成 Precondition 预读确认
- builder 确认报告可读可用后，leader 立即向该 builder 已确认覆盖且后续不再需要的 researcher 发送结构化 `shutdown_request`
- builder 产物通过 leader 验收后，leader 立即向该 builder 发送结构化 `shutdown_request`，不等其他 builder 或最终 closeout
...
5. 任一报告缺失、无法解析或结构不完整时，必须先解析 gate 输出并按 `researcher-<domain>.yaml` 归类失败原因，只处理失败 domain
6. 对每个失败 domain，优先通过 `SendMessage` 通知原 researcher 修复同名 yaml：
   - 消息必须包含 gate 原始失败摘要、目标文件路径、本次 `embed_stamp`、本次 `reports_dir`
   - 要求 researcher 只修复自己的 `researcher-<domain>.yaml`，不得重做无关 domain，不得写新 reports_dir
   - 修复仍使用原子写模式覆盖同名 yaml，完成后只发送 `report_path + status + summary`
7. retry 期间禁止再次 `TeamCreate("seed-embed")`，禁止全量重派所有 researcher
8. 只有原 researcher 不可用、拒绝修复或修复等待超过 `waveTimeoutSeconds` 时，才允许为该失败 domain 单独补派 1 个 replacement researcher
9. replacement researcher 仍写回同一 `reports_dir` 下的同名 yaml；每个失败 domain 最多补派 1 次
10. 修复或补派完成后，只对失败 domain 重新运行 Node gate；全部通过后再运行一次覆盖所有激活 domain 的最终 Node gate
11. 如果定向修复或单 domain 补派后仍失败：
...
builder 启动后必须先读取自己的 Precondition 指定报告，并立即 SendMessage 给 leader 发送预读确认：

- `confirmed_reports`: 已成功读取并可解析的报告路径
- `missing_or_invalid_reports`: 缺失或不可解析的报告路径；若非空，leader 必须回到 Phase A Gate 定向修复，不得继续生成
- `covered_researchers`: 本 builder 已确认覆盖的 researcher 名称

leader 收到 builder 的预读确认且 `missing_or_invalid_reports` 为空后，立即关闭 `covered_researchers` 中后续不再需要的 researcher。随后 builder 再按报告内容为每个目标 `matrix_id` 独立生成 skill。Phase B 中 builder 不依赖 mailbox 内联报告。
...
- builder 必须等待 leader 确认其预读结果；若 leader 要求回到 Phase A Gate 修复报告，builder 暂停生成
