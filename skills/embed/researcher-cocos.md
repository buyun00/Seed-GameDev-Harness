...
4. `$CLAUDE_PLUGIN_ROOT/skills/embed/researcher-cocos.md`
...
Deliverable: 单个 Cocos 矩阵项调查报告；写入 `.seed/state/embed/<embed_stamp>/reports/<matrix_id>.yaml`（原子写）；写完后 SendMessage 通知 leader 路径 + matrix_id + 状态摘要
...
- 本文件是 Cocos researcher profile，不代表一次任务要扫描全部 Cocos 方向。
...
- 只加载当前 `fixed_question_file` 与匹配 composite fixed question 文件，不得加载其它 Cocos 方向的问题。
