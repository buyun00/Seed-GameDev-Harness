...
- **Fixed Timestep**: Physics runs in FixedUpdate at a fixed interval (Edit > Project Settings > Time). All physics logic should be in `FixedUpdate()`, not `Update()`.
...
- **Layer Collision Matrix**: 检查 Edit > Project Settings > Physics 的碰撞矩阵，某些层之间可能被禁用了碰撞。
...
- **Root Motion vs Script-driven**: `Animator.applyRootMotion` 开启时，动画会控制 Transform 位移，和脚本的位移控制冲突。通常二选一。
- **Animation Events**: 动画事件触发时，如果目标 MonoBehaviour 被禁用或销毁，会静默失败，不会报错。
